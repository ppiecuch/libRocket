...
void GraphicSystem::resize(const int _width, const int _height, const float _scale)
{
    width = _width;
    height = _height;
    glViewport(0, 0, width * _scale, height * _scale);
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
...
unsigned char * GraphicSystem::loadTGA(const QString &path, Rocket::Core::Vector2i &texture_dimensions)
{
    ...
    Rocket::Core::FileHandle file_handle = file_interface->Open(path.toLatin1().data());
...
Vector2f GraphicSystem::scissorOffset(0, 0);
float GraphicSystem::scaleFactor = 1;
int GraphicSystem::width;
int GraphicSystem::height;
QStringList GraphicSystem::searchPaths;
